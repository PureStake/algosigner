...
export function Validate(field: any, value: any): ValidationResponse {
    switch(field) {
        ...
        case "to":
            if(!algosdk.isValidAddress(value)) {
                return new ValidationResponse({status:ValidationStatus.Invalid, info:'Address does not adhear to a valid structure.'});
            }
            else {
        ...
        case "voteKeyDilution":
            if (value && (!Number.isSafeInteger(value) || parseInt(value) < 0)){
                return new ValidationResponse({status:ValidationStatus.Invalid, info:'Value unable to be cast correctly to a numeric value.'});
            }
            else {
                return new ValidationResponse({status:ValidationStatus.Valid});  
            }
        ...
        case "fee":
            try {
                ...
                if(!Number.isSafeInteger(value) || parseInt(value) < 0) {
                    ...
                    return new ValidationResponse({status:ValidationStatus.Dangerous, info:'The associated fee is very high compared to the minimum value.'});
                }
                else if(parseInt(value) > 1000) { 
                    return new ValidationResponse({status:ValidationStatus.Warning, info:'The fee is higher than the minimum value.'});
                }      
                else {
                    return new ValidationResponse({status:ValidationStatus.Valid});  
                }
        ...
        case "genesisID":
            if(getSupportedLedgers().some(l => value === l["genesisId"])){ 
                ...
                return new ValidationResponse({status:ValidationStatus.Invalid, info:'The associated genesis id does not match a supported ledger.'}); 
            } 
        ...
        case "genesisHash":
            if(value) { 
                ...
                return new ValidationResponse({status:ValidationStatus.Invalid, info:'The genesis hash value is invalid or does not exist.'}); 
            }  
    ...
    return new ValidationResponse({status:ValidationStatus.Invalid, info:'An unknown error has occurred during validation.'}); 
}