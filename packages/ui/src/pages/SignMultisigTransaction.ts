...
const SignMultisigTransaction: FunctionalComponent = (props) => {
  ...
  useEffect(() => {
    ...
    chrome.runtime.onMessage.addListener((request, sender: any) => {
      ...
      if(isFromExtension(sender.origin) && request.body.method == JsonRpcMethod.SignMultisigTransaction) {
        setRequest(request);
    ...
    <div class="main-view" style="flex-direction: column; justify-content: space-between;">
      ...
      <div style="flex: 1; overflow:auto; max-height: 400px;">
        ${ request.body && html`
          ...
          ${ request.body.params.txn.type==="pay" && html`
            <${TxPay} tx=${request.body.params.txn} vo=${request.body.params.validityObject} account=${account} ledger=${ledger} />
          `}
          ${ request.body.params.txn.type==="keyreg" && html`
            <${TxKeyreg} tx=${request.body.params.txn} vo=${request.body.params.validityObject} account=${account} ledger=${ledger} />
          `}
          ${ request.body.params.txn.type==="acfg" && html`
            <${TxAcfg} tx=${request.body.params.txn} vo=${request.body.params.validityObject} dt=${request.body.params.txDerivedTypeText} account=${account} ledger=${ledger} />
          `}
          ${ request.body.params.txn.type==="axfer" && html`
            <${TxAxfer} tx=${request.body.params.txn} vo=${request.body.params.validityObject} dt=${request.body.params.txDerivedTypeText} account=${account} ledger=${ledger} />
          `}
          ${ request.body.params.txn.type==="afrz" && html`
            <${TxAfrz} tx=${request.body.params.txn} vo=${request.body.params.validityObject} account=${account} ledger=${ledger} />
          `}
          ${ request.body.params.txn.type==="appl" && html`
            <${TxAppl} tx=${request.body.params.txn} vo=${request.body.params.validityObject} account=${account} ledger=${ledger} />
          `}
          </section>
        `}
      </div>
