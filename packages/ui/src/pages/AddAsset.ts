    ...
    switch (tab) {
      case 0:
        if (verifiedResults.results) {
          ...
          setResults(verifiedResults.results.filter((elem) => {
            return (elem['asset_id'] === +value) ||
    ...
    <div class="main-view" style="flex-direction: column; justify-content: space-between;">
      ...
      <div style="flex: 1;">
        ...
        ${results.map((asset: any) => html`
          <div class="py-2 px-4 is-flex" onClick=${() => setSelectedAsset(asset)}
            ...
            <div>
              ${ asset.name && asset.name.length > 0 && html`
                ${ asset.name }
              `}
              ${ verifiedIDs.includes(asset['asset_id']) && html`
              ...
              ${ asset['unit_name'] && asset['unit_name'].length > 0 && html`
                <span class="has-text-grey-light"> ${asset['unit_name']}</span>
              `}
            </div>
        ...
        `)}
        ${ nextToken && nextToken.length > 0 && ! loading && html`
          <div class="py-2 px-4 has-text-centered"
