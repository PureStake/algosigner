...
const tx = {
  ...
  'signature': {
    sig:
      'lO/DZnJHH2iS/gReqIHzyJS2A36oPGJqx6hvHu0s5Ie/Didp/OMfRFHP4eUHJmzjMLZ0kf0S+OShxSoiimVRDQ==',
  },
  'tx-type': 'acfg',
};
...
describe('TxAcfg', () => {
  beforeEach(() => {
    component = shallow(html` <${TxAcfg} tx=${tx} ledger=${ledger} /> `);
  });
  ...
  it('should display asset name', () => {
    expect(
      component.contains(
        html`<span>${tx['asset-config-transaction']['params']['name']}</span>`
      )
    ).toBe(true);
  });
