...
const tx = {
  ...
  'signature': {
    sig:
      'TtL+XMfgNcQ1H/Tgdc11yFjLKm+dZ8PskkfTkxwgUih8FS4LW+ADQnmoVxDaolfwaX5km6XHBXY2Fx91e3o/DQ==',
  },
  'tx-type': 'afrz',
};
...
describe('TxAfrz', () => {
  beforeEach(() => {
    component = shallow(html` <${TxAfrz} tx=${tx} ledger=${ledger} /> `);
  });
  ...
  it('should display frozen address', () => {
    expect(
      component.contains(
        html`<span>${tx['asset-freeze-transaction']['address']}</span>`
      )
    ).toBe(true);
  });
  ...
  it('should display asset ID', () => {
    expect(
      component.contains(
        html`<span
          >${tx['asset-freeze-transaction']['asset-id'].toString()}</span
        >`
      )
    ).toBe(true);
  });
