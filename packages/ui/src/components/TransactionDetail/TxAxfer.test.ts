...
const tx = {
  ...
  "sender-rewards": 0,
  "signature": {
    "sig": "SP9PaE/7EkOrhBhieble5kcfHbbUGaDl2EXtPcL4OSZy5bPO/ruxECWJPECCH3TJBd9Z8nDMAnv0bZYp/DL8Bg=="
  },
  "tx-type": "axfer"
};
...
describe('TxAxfer', () => {
  ...
  it('should display receiver', () => {
    expect(component.contains(html`<span>${tx['asset-transfer-transaction'].receiver}</span>`)).toBe(true);
  });

  it('should display amount', () => {
    expect(component.contains(html`<span>${(tx['asset-transfer-transaction']['amount']).toString()}</span>`)).toBe(true);
  });
